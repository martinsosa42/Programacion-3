...
def main():
    ...
    print(f"\nEl resultado de la suma de la lista es: {suma_lista(lista)}")

    print(f"\nLos numeros menores a 5 en la lista son: {filtrar_menores(lista, 5)}")

    print(f"\nEl menor, central y mayor de la lista son: {valores(lista)}")

    print(f"\nUnicos set: {unicos_set(lista)}")

    print(f"\nUnicos dict1: {unicos_dict1(lista)}")
    print(f"\nUnicos dict2: {unicos_dict2(lista)}")
    print(f"\nUnicos dict3: {unicos_dict3(lista)}")

    print(f"\nHistograma: {histograma(texto)}")
